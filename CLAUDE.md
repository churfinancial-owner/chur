...
- **Card art is not in the app.** `Assets.xcassets/Cards` was deleted in P1b — images are published to the CDN and fetched on demand. **Never write `Image(card.imageName)` or `UIImage(named: card.imageName)`**; both return nothing. Use `CardArtView(imageName:)` (`SharedDesign/Components/`), which resolves memory → disk → CDN and shows a placeholder meanwhile. Issuer logos, merchant icons and badge icons *are* still bundled and are unaffected. The image files live at repo-root `CardArt/` (see Project layout). Five call sites survived the P1b conversion and rendered blank for a day, so when you add a render path, `grep -rn "Image(.*[Ii]mageName"` before believing it's the only one.
