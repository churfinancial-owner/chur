...
- **Buttons:** `ScaleButtonStyle` / `SquishyButtonStyle` from `Style.swift`; shared controls like `ChurDoneButton` (the sage confirm capsule — **every sheet's Done, toolbar or overlay**; it replaced 21 hand-rolled copies), `OliveRingIconButton` (ring-outline circular icon button; `OliveRingIcon` for use as a `Menu` label), `RatePill`, `SheetDismissButton`, `EmptyStatePlaceholder`, `WaveDivider` live in `SharedDesign/Components/` — reuse before creating new ones.
