...
- **Buttons:** `ScaleButtonStyle` / `SquishyButtonStyle` from `Style.swift`; shared controls like `ChurDoneButton` / `ChurCancelButton` (**every sheet's Done and Cancel, toolbar or overlay** — `churSageDeep` for confirm, `churRoseDeep` for cancel, so backing out never carries the same weight as confirming; they replaced 30 hand-rolled copies). **Default `emphasis: .toolbar` is colour only** — iOS 26 wraps each toolbar item in its own glass container, so drawing a capsule there nests two containers and a tinted fill washes out against the glass. Use `.prominent` only for buttons outside a toolbar. Alert buttons using `role: .cancel` are system-styled and stay as they are, `OliveRingIconButton` (ring-outline circular icon button; `OliveRingIcon` for use as a `Menu` label), `RatePill`, `SheetDismissButton`, `EmptyStatePlaceholder`, `WaveDivider` live in `SharedDesign/Components/` — reuse before creating new ones.
