...
- **Buttons:** `ScaleButtonStyle` / `SquishyButtonStyle` from `Style.swift`; shared controls like `OliveIconButton`, `RatePill`, `SheetDismissButton`, `EmptyStatePlaceholder`, `WaveDivider` live in `SharedDesign/Components/` — reuse before creating new ones.
