...
- `Chur/ROADMAP.md` — growth priorities (P0 schema migration ✅ → P1 remote content pipeline ✅, P1c test vectors ✅ / contract spec deferred → P1d everything-else-goes-remote ✅ → P1e design system pass ✅ → **P1f Hong Kong reward mechanics, in progress** → P2 analytics → P3 annual card value → P4 widget), the Visa API assessment, and market sequencing. **Read before starting new feature work so it lands in priority order; update it whenever priorities shift or a phase completes.** Its "P1b — lessons worth keeping" section is the post-mortem for the whole content pipeline — read it before changing publishing, card art, or content refresh.
