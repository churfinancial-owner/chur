...
- **Popup headers with a corner watermark:** `PopupHeaderWatermark` + `PopupHeaderMetrics` + the `.popupHeaderTitle()` / `.popupHeaderInset()` modifiers (`Core/RewardComponents/RatePopupComponents.swift`) are the shared treatment for the merchant popup, category popup and benefit detail sheet — a tinted circle bleeding off the top-trailing corner, with header text inset `PopupHeaderMetrics.contentInset` so it clears the mark. **Never write the inset as a literal and never restyle a header title inline**; both drifted across three screens before this was extracted. A header carrying the watermark also needs `.frame(maxWidth: .infinity)`, or `RepeatingPatternBackground` renders only as wide as the widest line.
