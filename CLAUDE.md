...
- **Buttons:** `ScaleButtonStyle` / `SquishyButtonStyle` from `Style.swift`; shared controls like `ChurDoneButton` / `ChurCancelButton` (**every sheet's Done and Cancel, toolbar or overlay** — filled `churSageDeep` for confirm, `churRoseDeep` tinted for cancel, so backing out never carries the same weight as confirming; they replaced 30 hand-rolled copies). Alert buttons using `role: .cancel` are system-styled and stay as they are, `OliveRingIconButton` (ring-outline circular icon button; `OliveRingIcon` for use as a `Menu` label), `RatePill`, `SheetDismissButton`, `EmptyStatePlaceholder`, `WaveDivider` live in `SharedDesign/Components/` — reuse before creating new ones.
