...
- `Chur/ROADMAP.md` — growth priorities (P0 schema migration blocker → P1 remote content pipeline → P2 analytics → P3 annual card value → P4 widget), the Visa API assessment, and market sequencing. **Read before starting new feature work so it lands in priority order; update it whenever priorities shift or a phase completes.**
...
- **Card art is not in the app.** `Assets.xcassets/Cards` was deleted in P1b — images are published to the CDN and fetched on demand. **Never write `Image(card.imageName)` or `UIImage(named: card.imageName)`**; both return nothing. Use `CardArtView(imageName:)` (`SharedDesign/Components/`), which resolves memory → disk → CDN and shows a placeholder meanwhile. Issuer logos, merchant icons and badge icons *are* still bundled and are unaffected.
- **Buttons:** `ScaleButtonStyle` / `SquishyButtonStyle` from `Style.swift`; shared controls like `OliveRingIconButton` (ring-outline circular icon button; `OliveRingIcon` for use as a `Menu` label), `RatePill`, `SheetDismissButton`, `EmptyStatePlaceholder`, `WaveDivider` live in `SharedDesign/Components/` — reuse before creating new ones.
- **Tool sheet banners:** `ToolSheetHeaderBanner` (`SharedDesign/Components/ToolSheetHeaderBanner.swift`) is the shared header for full-screen tool sheets and popups (Couponing, Transfer Partners, Year/Month Summary, Benefit/Merchant/Category detail sheets) — off-white background, olive dot/SF-Symbol pattern via `RepeatingPatternBackground`, grab handle, and `SheetDismissButton` baked in. Callers only supply title/pill/subtitle content. Reuse it for any new tool sheet instead of building a bespoke header.
