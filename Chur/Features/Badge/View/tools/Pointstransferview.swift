            ...
            LazyVStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 28) {
                    ...
                    if !filteredBanks.isEmpty {
                        partnerSection(title: AppLocale.string("POINT SOURCES"), icon: "building.columns.fill", items: filteredBanks, color: .blue, isBank: true)
                    }

                    if !filteredAirlines.isEmpty {
                        partnerSection(title: AppLocale.string("AIRLINES"), icon: "airplane", items: filteredAirlines, color: .purple, isBank: false)
                    }

                    if !filteredHotels.isEmpty {
                        partnerSection(title: AppLocale.string("HOTELS"), icon: "bed.double.fill", items: filteredHotels, color: .orange, isBank: false)
                    }
