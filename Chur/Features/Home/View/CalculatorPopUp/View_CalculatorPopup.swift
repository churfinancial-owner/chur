...
struct CalculatorPopup: View {
    ...
    private func isCrossBorderSpend(for card: CreditCard) -> Bool {
        guard let merchantRegion = normalizedRegionCode(merchant.region),
              let cardRegion = normalizedRegionCode(card.country) else {
            return false // Unknown region: skip FX penalty.
        }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ForEach(Array(categoryHierarchy.enumerated()), id: \.offset) { index, catID in
                                ...
                                HStack(spacing: 8) {
                                    ...
                                    Text(catID)
                                        .font(.system(size: 13, weight: index == 0 ? .bold : .regular, design: .rounded))
