...
struct CardOrderSheet: View {
        ...
        NavigationStack {
            VStack(spacing: 0) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("Drag the handles to reorder. The top card is your primary display.")
                        .font(.churFootnote())
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Done") {
                        saveOrder()
                        dismiss()
                    }.foregroundStyle(Color.churOlive).fontWeight(.bold)
                }
            }
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 6) {
                    ...
                    if isPrimary {
                        Text("PRIMARY").font(.system(size: 8, weight: .bold)).foregroundStyle(.white).padding(.horizontal, 4).padding(.vertical, 2).background(Color.churOlive).clipShape(Capsule())
                    }
                }
    ...
    private func applySort(_ criteria: SortCriteria) {
        if currentSort == criteria {
            ...
            // Name/Issuer: First click = A-Z (Chevron Up)
            // Fee/Approved: First click = High/Newest (Chevron Down)
        ...
        withAnimation(.spring(response: 0.4, dampingFraction: 0.8)) {
            ...
            switch criteria {
            case .name:
                // Mirrored: > results in A-Z visually
            ...
            case .annualFee:
                // Mirrored: < results in High-to-Low visually
