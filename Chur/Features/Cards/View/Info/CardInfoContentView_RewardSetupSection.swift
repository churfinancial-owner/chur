...
struct RewardSetupSection: View {
    ...
    var boostDisplay: String {
        guard let program = card.boostProgram, let tier = user?.boostEnrollments[program.id] else { return AppLocale.string("Not enrolled") }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                if card.boostProgram != nil {
                    CardRowDivider()
                    DetailRow(label: AppLocale.string("Relationship Boost"), value: boostDisplay, isEditable: true) {
                        activeSheet = .boost
                    }
                }
            }
