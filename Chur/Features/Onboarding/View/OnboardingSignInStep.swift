    ...
    private func performGoogleSignIn() {
        ...
        GIDSignIn.sharedInstance.signIn(
            ...
            if let error = error {
                ...
                UINotificationFeedbackGenerator().notificationOccurred(.error)
                signInError = AppLocale.string("Google sign in failed.")
                return
            }
...
private struct BackupTooltipContent: View {
    private let items: [(String, String)] = [
        ("creditcard.fill",      AppLocale.string("Cards, approved dates & notes")),
        ("checkmark.seal.fill",  AppLocale.string("Benefit history & usage")),
        ("slider.horizontal.3",  AppLocale.string("Preferences & settings")),
    ]
