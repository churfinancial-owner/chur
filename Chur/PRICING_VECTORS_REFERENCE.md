...
- **`BoostProgramDatabase` reads `boost_programs.json` from `Bundle.main`.** The boost vector needs a real program id, a real tier name and a real eligible `templateID` (`us-bank-smartly` / `50% Smartly Earning Bonus`); the layer vectors need `hsbc-hk-travel-guru` and `hsbc-hk-red-hot-rewards`, which are scoped by `eligibleIssuer` + `eligibleCountry`, so their fixture cards carry `issuer: "HSBC"`, `country: "HK"`. If the host app isn't set, the lookup returns `nil`, no layer fires and the vector fails — loudly, which is the right failure. The runner pins the host app to the bundle for each vector (`DebugOverrides.forceBundledContent`), because a cached CDN copy of `boost_programs.json` would otherwise win over one edited on the branch.
