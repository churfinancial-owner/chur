...
5. **`Debug/Testing/SeedDataValidator.swift`** extended to validate a candidate payload before publishing. Partly done: it now checks every card's benefit references against what `BenefitDatabase` actually loaded, which is what surfaced the ten silently-dropped benefits. Validating a *candidate* payload before upload is still outstanding — **and is the next item worth doing**, see below.

**The remaining P1b item, restated after 2026-08-14.** The gap is no longer "nice to have a validator": it is that `RemoteContentService.validate(_:for:)` and `ChurContentPublish` check the same bundles by different rules, so the strict one only ever runs on a device. An empty `cardArt` index passed the publisher, uploaded, and took every other domain down with it. The fix is one rule set applied twice — the publisher should refuse to upload anything the app would refuse to apply. Guards added on the day (empty art index, duplicate `imageName`) are the ad-hoc version of exactly this, and each was written only after the corresponding failure.
