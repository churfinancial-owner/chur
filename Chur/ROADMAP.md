...
| 5a | Points and miles: cost per mile parsed from the authored `ratio`; `TransferPartnerDatabase.allPrograms` so a card from another market still resolves; `CreditCard.transferRoutes`; a Points and Miles section on card info, including the Freedom case where an upgrade card would unlock transfers; the `perMile` style becomes the paired `4% (HK$2.5/里)` and is gated on the program having partners; a program may declare the bank's handling `fee` per redemption (Citi HK charges HK$200), rendered by the same `ConditionText` as gates and caps and equally display-only | authored 2026-09-11; no engine change |
