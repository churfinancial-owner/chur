...
| 4 | Conditions as information: `gate` and `cap` on `RewardRate` using the layer struct; localized small print in `EarningRatesSection` and the card info rows; `rateStyle` per program | authored 2026-09-11; one `ConditionText` formatter for all three sites; HK cash programs read as `%` and miles programs as `HK$x = 1 mile`; no engine change, no new vectors; awaiting Mac build + fingerprint |
