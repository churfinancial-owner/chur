...
| Rate expressed as HK$ per mile | DBS "HK$2 = 1 mile", Amex "HK$1.68 = 1 mile" | `rateStyle` per program in `SeedDataPrograms.json`, display only. Authoring rule: `rate` is **miles per dollar** (HK$4 = 1 mile is `0.25`) against a point value of what a mile is really worth (~HK$0.10). Getting both wrong in opposite directions cancels out in the ranking and hides indefinitely — `sc-hk-cathay` shipped that way |
...
| 2 | Earning layers: extend `BoostProgram` in place; lookup returns every layer for a card; engine stacks them; `boostEnrollments` becomes structured; `BoostProgramPickerSheet` gains allocate and pick-one modes; the four display sites that multiply by a boost double take an applied-layers value instead | ✅ 2026-09-11, 37 vectors green on the Mac; Travel Guru + Red Hot Rewards in `boost_programs.json`; Bonus Programs section on card info; hammer-menu "Use Bundled Content" pin added after the CDN copy hid the new programs |
| 3 | Transaction dimensions: `channels`, `currencies`, `excludedCountries`, `paymentMethods` on `RewardRate` and on layers; currency derived in the engine; `contactless` and the HK wallet ids added to the payment vocabulary; decoders (`SeedDataLoader`, `CardDatabase`, `CardSyncService.updateRewardFields`) and the vector fixture updated | ✅ 2026-09-11, 46 vectors green on the Mac; `PaymentMethods` vocabulary + 4 hidden payment categories; `CurrencyConversion.currencyCode(forRegion:)` with a Locale fallback; Travel+ and Citi Rewards HK authored on the new fields as the proof; multi-region online merchants bill local cards locally |
| 4 | Conditions as information: `gate` and `cap` on `RewardRate` using the layer struct; localized small print in `EarningRatesSection` and the card info rows; `rateStyle` per program | ✅ 2026-09-11, build green on the Mac; one `ConditionText` formatter for all three sites; HK cash programs read as `%` and miles programs as `HK$x = 1 mile`; no engine change, no new vectors |
| 5a | Points and miles: cost per mile parsed from the authored `ratio`; `TransferPartnerDatabase.allPrograms` so a card from another market still resolves; `CreditCard.transferRoutes`; a Transfer Partners row in Reward Setup showing the partner count with the handling fee underneath, opening the transfer sheet on that card's own program; the sheet gains a flag picker across the markets the wallet reaches, shown only when there is more than one; the `perMile` style becomes the paired `4% (HK$2.5/里)` and is gated on the program having partners; a program declares the bank's handling `fee` per redemption (Citi HK charges HK$200) and a card template overrides it for waivers, rendered by the same `ConditionText` as gates and caps and equally display-only | ✅ 2026-09-12, build green on the Mac; also fixed a stale-payload bug where a sheet opened from a row lost its argument intermittently — the payload now rides on the `ActiveSheet` case |
| 5c | The display seam, unplanned — fallout from the first miles card. `RateDisplay` replaces eight formatters that had grown apart: three knew the reward program and three did not, two produced the effective percentage with different rules for zero, and whether the Card Rate / Effective Rate preference was honoured depended on the screen. Sub-1.0 rates were also filtered out of every earning-rate list by a `rate > 1.0` test that assumed multipliers, and point values printed as bare `¢` in two currencies | ✅ 2026-09-15, build green on the Mac. Two more formatters surfaced after the first sweep and are folded in: `perMile` dropped its paired percentage so a card rate states the rate and nothing else (which collapsed the three display contexts to two), and `RatePill`'s convenience init was formatting rates itself inside the design system — no reward program, so Earning Power printed Asia Miles as `0.2x`. It also rounded sub-1 rates to zero, which made MMPower's 0.4% base read as `0%`. **The lesson: a sweep for sites that format a rate misses sites that hand numbers to a component and let it format.** |
| 5b | Prove it on HK content (add a `home` category for Red Hot's 賞家居 and put it in the allocation): re-author the HSBC, Hang Seng, Citi and Standard Chartered reward JSON onto the new shapes, miles rates as miles per dollar; verify upgrade-rule coverage for Citi ThankYou and Wells Fargo; add the HK programs' transfer partners; HK vectors for additive stacking, allocation weights, currency lists, EEA exclusion, online-overseas exclusion, contactless and e-wallet exclusion; update the five reference docs | |
