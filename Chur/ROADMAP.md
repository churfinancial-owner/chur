...
| 5a | Points and miles: cost per mile parsed from the authored `ratio`; `TransferPartnerDatabase.allPrograms` so a card from another market still resolves; `CreditCard.transferRoutes`; a Points and Miles section on card info, including the Freedom case where an upgrade card would unlock transfers; the `perMile` style becomes the paired `4% (HK$2.5/里)` and is gated on the program having partners | authored 2026-09-11; no engine change |
