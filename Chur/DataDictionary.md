...
`SeedDataTransferPartners.json` is keyed by `programName`, the same string `RewardRate.rewardProgramName` carries, which is what makes "does this card earn miles" answerable without a field on the card. `ratio` is authored `"points:miles"` (`"1:1"`, `"1:1.6"`) and parsed into `milesPerPoint`; the cost of one mile is the program's `pointCashValue` divided by that. Since P1f part 5 the database keeps two tables: `programs`, filtered to the user's region, and `allPrograms`, unfiltered, which a specific card's info screen reads so a card from another market still resolves. The Points Transfer sheet computes its own tables with `derived(forRegion:)` and offers a flag picker across `regions(forWallet:)` — the markets the wallet actually reaches — so a user holding both US and HK cards can see either market's partners without changing their region setting. One market means no picker.

A program may carry a `fee` (`{amount, currency, basis, note}`) for the bank's handling charge on a redemption, e.g. Citi HK's HK$200. `basis` is `redemption` (default) or `transaction`. A **card template** may carry its own `transferFee`, which wins — a premium card with the fee waived authors `{"amount": 0, "note": "Waived"}` and the row reads "Waived" rather than a figure. The fee lives on the template, not the SwiftData model, because it is static content the user never edits. **Display only**, like a gate or a cap: the real cost per mile depends on how many points move, which the app does not know.
