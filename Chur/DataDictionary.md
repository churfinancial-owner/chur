...
A program may carry a `fee` (`{amount, currency, basis, note}`) for the bank's handling charge on a redemption, e.g. Citi HK's HK$200. `basis` is `redemption` (default) or `transaction`. **Display only**, like a gate or a cap: the real cost per mile depends on how many points move, which the app does not know.
